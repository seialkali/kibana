...
import { EisUpdateCallout, type EisUpgradeCalloutProps } from './eis_update_callout';
...
import {
  ...
  EIS_UPGRADE_CALLOUT_DESCRIPTION,
  EIS_UPGRADE_CALLOUT_CTA,
...
describe('EisUpdateCallout', () => {
  ...
  const direction: EisUpgradeCalloutProps['direction'] = 'row';

  const mockOnDismissTour = jest.fn();
  const mockHandleOnClick = jest.fn();

  const renderEisUpdateCallout = (props?: Partial<EisUpgradeCalloutProps>) => {
    return render(
      <EuiThemeProvider>
        <EisUpdateCallout
          promoId={promoId}
  ...
  it('renders callout when promo is visible', () => {
    ...
    expect(screen.getByText(EIS_UPGRADE_CALLOUT_DESCRIPTION)).toBeInTheDocument();

    expect(screen.getByText(EIS_UPGRADE_CALLOUT_CTA)).toBeInTheDocument();
  ...
  it('calls handleOnClick when CTA button is clicked', () => {
    ...
    const ctaButton = screen.getByTestId('eisUpgradeCalloutCtaBtn');
