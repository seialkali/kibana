...
jest.mock('../../../public/application/components/mappings_editor/lib/utils', () => ({
  getAllElserFields: jest.fn(),
  deNormalizeSelectedElserMappings: jest.fn(),
...
jest.mock('../../../public/application/services/notification', () => ({
  notificationService: { showSuccessToast: jest.fn() },
}));

const getAllElserFieldsMock = jest.mocked(getAllElserFields);
const deNormalizeSelectedElserMappingsMock = jest.mocked(deNormalizeSelectedElserMappings);
...
describe('UpdateElserMappingsModal', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    // Default mock for mappings state
    ...
    getAllElserFieldsMock.mockReturnValue([
      {
        label: 'name',
        key: 'first',
    ...
    deNormalizeSelectedElserMappingsMock.mockReturnValue({
      name: {
        type: 'semantic_text',
  ...
  it('should display error message if API fails', async () => {
    ...
    updateIndexMappingsMock.mockResolvedValue({
      error: { message: 'Something has gone wrong', error: 'Something has gone wrong' },
      data: null,
    });

    const applyBtn = screen.getByTestId('UpdateElserMappingsModalApplyBtn');
    await userEvent.click(applyBtn);
    expect(await screen.findByText(`Error updating mapping: ${errorMessage}`)).toBeInTheDocument();
